...
import { Scene3D } from "../ui/Scene3D";
import SpeechRecognition, { useSpeechRecognition } from "react-speech-recognition";
import { reviewAnswer, generateFinalReview } from "../ai_api/grok";
...
export default function Interview() {
  ...
  if (!browserSupportsSpeechRecognition) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-black text-white">
        Browser does not support SpeechRecognition. Use Chrome/Edge.
      </div>
    );
  }
  ...
  useEffect(() => {
    ...
    const startTime = getTimeForLevel(currentQuestion.level);
  ...
  const handleNext = async (auto = false) => {
    ...
    const result = await reviewAnswer({
      ...
      answer: transcript || manualAnswer || (auto ? "[Auto-submitted: Time expired]" : ""),
    });

    dispatch(
      saveAnswer({
        ...currentQuestion,
        answer: transcript || manualAnswer,
        score: result.score,
        notes: result.notes,
        timeTaken: getTimeForLevel(currentQuestion.level) - timeLeft,
      })
    );
  ...
  const handleSubmit = async () => {
    setLoading(true);
    const candidateData = { ...session.currentCandidate, answers: [...session.answers] };
    const finalReview = await generateFinalReview({ candidate: candidateData });
    ...
    dispatch(completeSession(finalReview));
    dispatch(addCandidate(completedData));
      ...
      <main className="flex-1 flex flex-col md:flex-row gap-6 p-6 relative z-10">
        ...
        <div className="w-full md:w-1/3 bg-white/10 backdrop-blur-md border border-white/10 rounded-xl shadow p-6 flex flex-col">
          ...
          <textarea
            ...
            className="flex-1 p-3 border rounded-lg bg-black/30 text-sm text-gray-200"
          />

          <button
            onClick={() => handleNext(false)}
            disabled={loading}
            className="mt-6 py-2 bg-gradient-to-r from-indigo-500 to-emerald-500 text-white rounded-lg font-semibold hover:shadow-lg transition"
          >
            {loading ? (
